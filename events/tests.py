...
class EventRegistrationFormTests(TestCase):
    ...
    def test_registration_api_saves_valid_json_data(self):
        response = self.client.post(
            reverse('event_register_api'),
    ...
    def test_registration_api_returns_validation_errors(self):
        response = self.client.post(
            reverse('event_register_api'),
            data=json.dumps(
                self.valid_data(full_name='Ana', email='ana@example.com', age=16),
            ),
            content_type='application/json',
        )
    ...
    def test_registration_api_rejects_invalid_json(self):
        response = self.client.post(
            reverse('event_register_api'),
            data='{',
            content_type='application/json',
        ...
        self.assertEqual(response.status_code, 400)
        self.assertFalse(response.json()['success'])
        self.assertIn('request', response.json()['errors'])

    @override_settings(CORS_ALLOWED_ORIGINS=['https://example.vercel.app'])
    def test_registration_api_options_returns_cors_headers(self):
